...
// Явная специализация шаблона для тестов
...
protected:
    ...
    // Методы для доступа к приватным членам (если они нужны)
    // В текущей реализации TBSTTable не предоставляет доступа к приватным методам BST
};
